...
function! s:on_newline()"{{{
  ...
  if l:prev_line =~# '\<where\>'
    if l:prev_line =~# '^\s*\<where\>\s*$'
      return indent(l:prev_lnum) + &l:shiftwidth / 2
    elseif l:prev_line =~# '^\s*\<where\>'
      return matchend(l:prev_line, '\<where\>\s\+')
    endif
    ...
    while l:lnum >= 1
      ...
      if l:line =~# '^\s*\(\<class\>\|\<instance\>\)'
        return indent(l:lnum) + &l:shiftwidth / 2
      elseif l:line =~# '^\s*\(\w\|$\)'
        break
      endif
      let l:lnum -= 1
